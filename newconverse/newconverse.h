...
! talkoptions.top_menu - set to true if dialogue choices should be at top of
!  screen
...
routine SayAssertion(theAssert, showPrompt)
{
	...
	if speaking
	{
		...
		SetCurrentTopic(speaking.current_topic)	! resetting the current topic
																...
																!  assertion list
		AnyTopicsLeft
		if not talkoptions.gNumAvailableAssertions and not talkoptions.auto and
			talkoptions.conv_force and not talkoptions.topic_change
		{
			local a
			""
...
routine ConverseMessage(r, num, a, b)
{
	...
	#ifset _PASTTENSE_H
				if pasttense
					"Nothing else about that topic came to mind."
				else
#endif
				"Nothing else about this topic comes to mind."
			}
	}
...
replace PrintStatusline
{
	...
	window display.statusline_height
	{
		...
		if display.statusline_height ~= 2
		{
         ...
         text to _temp_string
		   if STATUSTYPE = 1 and (speaking is not chatmenu or
			not talkoptions.top_menu)
				print number score; " / "; number counter;
		   elseif STATUSTYPE = 2 and (speaking is not chatmenu or
			not talkoptions.top_menu)
			...
			if (speaking is chatmenu and talkoptions.top_menu)
			or (STATUSTYPE ~= 0 and (speaking is not chatmenu or
			not talkoptions.top_menu))
...
routine ListOptions
{
	if not talkoptions.gNumAvailableAssertions and not ( AnyTopicsLeft and
		talkoptions.topic_change)
		return
	...
	if talkoptions.show_topics
	{
		...
		{
			""
			ConverseMessage(&ListOptions,3) ! "Nothing else about this
			                                !  topic comes to mind."
			""
		}
		else
...
routine AnyTopicsLeft(new)
{
	...
	{
		for a in speaking.current_topic
		{
			for b in a
			{
				if b.Prerequisite and (b.only_npc = speaking or not b.only_npc)
		...
		if d and ((new and not speaking.current_topic) or
			talkoptions.auto)
		{
				if not new
				"\n";
		...
		elseif not talkoptions.topic_change
		{
			"\n";
